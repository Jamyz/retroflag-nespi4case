...
wget -N -q --show-progress "https://raw.githubusercontent.com/Jamyz/retroflag-nespi4case/master/SafeShutdown.py"
#-----------------------------------------------------------
...
if ! grep -q "^dtoverlay=gpio-poweroff,gpiopin=4,active_low=1,input=1" $File; then
    ...
    echo "# Overlaysudo  setup for proper powercut, needed for Retroflag cases" >> "$File"
